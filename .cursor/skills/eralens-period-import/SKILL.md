...
- **年精度顺序继位切年**（先秦通行，与英文维基国王表 / 逾年改元一致；实现见 `data/imports/lib/deathYearSuccession.mjs`）：
  ...
  - **例外（保持同年，不后移）**：
    ...
    3. 真正并立 / 旁支（`claim_track`、曲沃与翼、田齐太公与姜齐康公）；
...
`persons.name` 用可检索的常用名（禹、姬发、孔子、韦后）。**时间轴卡片主行显示 `persons.name`，因此君主姓名须带姓**（如莒郊公写 `己狂` 而非 `狂`，薛献公写 `任谷` 而非 `谷`）。维基诸侯表若只给「国君本名」，须结合该国姓氏（如莒己、滕姬、杞姒）补全；仅知谥号而本名失考时，可用 `{姓}{谥号}`（如 `姒武公`）。搜索只匹配 dynasty/person/event 的 `name`，不匹配 `title` 或谥号。
...
- `preferred_appellation` 仅用于 **regnal** 例外（先秦称号、秦襄公、西楚霸王等）；不要写入庙号/谥号/年号的默认 preferred。
