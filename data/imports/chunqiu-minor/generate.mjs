...
// Bronze inscription dating places the first six generations in specific
// Western Zhou reign eras. Do not spread them uniformly across the much longer
// founder-to-Spring-and-Autumn interval: that displaced 命仲 and 文 by centuries.
// Era boundaries follow the Xia-Shang-Zhou Chronology Project; ruler lifetimes
// within each broad era remain estimates, not attested accession/death years.
const tengEarlyReigns = [
  interpolatedTengReign("teng-r0", "滕错叔", -1045, -1043, "approximate", "interpolated"),
  interpolatedTengReign("teng-r20", "？", -1042, -1021),
  interpolatedTengReign("teng-r21", "？", -1020, -1001),
  interpolatedTengReign("teng-r22", "命仲", -1000, -990),
  interpolatedTengReign("teng-r23", "？", -989, -978),
  interpolatedTengReign("teng-r24", "滕伯文", -977, -950, "interpolated", null),
  ...interpolateTengReigns([
...
const manifest = {
  ...
  sources: [
    ...
    { label: "夏商周断代工程西周年表（周王年代）", url: "https://zh.wikipedia.org/wiki/周朝纪年" },
  ...
  notes: [
    ...
    "滕国早期世系按张志鹏《滕国新考》及金文断代校正：叔绣始封；二世在成康之际；三世在康王期；四世命仲在康昭之际；五世为命仲之子、滕伯文之父，在昭王期；六世滕伯文在昭穆之际。此前错误地将始封至前716年整体均分，造成前三处时代错位。前六世采用夏商周断代工程周王年代作为时代锚点，并以相邻时代之间的估算年份表达宽泛在位段；这些不是滕君确切即位/卒年，内侧边界标 interpolated。其后第七至十三世（滕侯毂）仅在六世之后至前716年卒年锚点间均分，标 interpolated。前484年隐公卒至前414年越灭滕依《世本》‘隐公后六世’分六段。",
