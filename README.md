...
事件层不重复解释王朝泳道。收录前先判断：如果只看王朝泳道、在位卡片和都城数据，这条信息是否仍然不可知？如果不能提供新增信息，就不建事件。

不收录常规的王朝更替、建国、改国号、称帝、禅让、普通即位退位，以及已经由王朝起止或 `claim_track` 表达的边界。例如「南齐代宋」「南梁代齐」只说明一条泳道结束、另一条泳道开始，应由泳道和在位卡表达。

优先收录四类事件：

- 泳道无法表达的疆域割让、收复、边界重划和长期控制权变化；
- 改变长期关系的跨政权同盟、和约、会盟；
- 泳道无法表达的政治象征、制度转折和社会结构变化；
- 少数特别重大的统一、分裂或战争转折，例如「秦统一中国」「隋灭陈统一」。

事件可以与王朝更替同时发生，但不能仅因为“某朝代替某朝”就入库。复杂疆域变化拆为多个有独立信息的节点；同一事实已有事件时，补 `event_dynasties` 或修正原事件，不另建同义事件。
