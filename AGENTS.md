...
4. 空白必须先定性，再决定是否写数据：
   ...
   - **年代失考**：知道是谁、年份为均分/约数 → `start/end_date_confidence`，仅两侧都失考且日历相接时画波浪线。
   - 禁止用相邻年份不连续自动插入「史料缺」。深度不够就继续查，不要标成缺载。
5. 王朝名用国号，一般不带「国」（齐、楚、汉赵）。例外须有通行称呼依据：战国「东周国」「西周国」（区别于西周/东周王室行）、滇国等。
6. `persons.name` 是卡片主行，君主须带姓（莒郊公写 `己狂` 不写 `狂`）。`posthumous_name` / `temple_name` 只存本体，不带国名前缀。国名写在 `title`。
7. 年精度占位月（1 或 12）不得展示为「×月」，以免假精确。界面 tooltip 用 compact 年份（`前221年`），不要写「公元」。
8. 金色是正统**覆盖色**，不是泳道本色。每个泳道有 `color_token` 本色；卡片在正统窗口内叠金。命运虚线、泳道底、左侧名牌一律用本色，禁止金色。

## 排查

1. 任何数据问题，必须搜索维基百科等权威资料核对，不能仅凭 `generate.mjs` 里的现成数字开工。
2. 先分清是数据、导入残留重复、还是渲染规则。页面上下叠卡优先查库内重复 id/同名宽跨度旧行，而不是先改 CSS。
3. 改精度、切年、正统窗口前，用原始史料起迄年判断，不要在已经后移过的日期上再切一次。

## 展示与布局

- **称谓**：`resolveEmperorAppellation` 按在位起始年——唐以前偏谥号/称号，唐至元偏庙号，明清偏年号。`preferred_appellation` 只用于 regnal 例外（先秦称号、秦襄公、西楚霸王等），不要为「默认该显示庙号」去写 preferred。
...
- **并存 cluster**（`dynasty_groups`）：三国、五胡、南北朝、五代十国等各占一行、贴在一起套框。框的起止用组自己的 span，不被成员更早建国年撑大。
...
- 并立 track、王莽/更始等非约定主线不上金。魏蜀吴称帝前不建帝王卡。
...
- **覆盖预期**：有明确灭国/禅代对象的王朝，末代通常应有一条线。无时间轴接收方（如吐蕃末代被僧人杀、回鹘亡于黠戛斯、南诏之后为大长和未收录）则不画，并在 manifest notes 说明。
