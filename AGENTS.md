...
- **称谓**：`resolveEmperorAppellation` 按在位起始年——唐以前偏谥号/称号，唐至元偏庙号，明清偏年号。`preferred_appellation` 只用于 regnal 例外（先秦称号、秦襄公、西楚霸王等），不要为「默认该显示庙号」去写 preferred。先秦（始皇帝以前）卡片主行改用该称谓，副行才是私名。
