...
3. 年精度顺序继位（先秦通行）：死年整年归旧王，新王从下一年起算。一年短祚、史料写明未逾年改元、真正并立除外。月日有史料则用月日。不要把维基「在位年份」的死年重叠画成两人并立。
   - 所有相邻时间区间的对象归属统一从 `packages/shared/src/timelineOwnership.ts` 进入，底层由 `timelineIntervals.ts` 按 `year` / `month` / `day` 精度展开到日历日并执行左开右闭裁定，混合精度及单条记录起止精度不同也走同一入口。重叠部分归较早记录，后记录从前记录结束后的第一个日历日开始。该入口集中负责在位、同地点同角色都城、合并泳道相位的接续分组，以及事件、命运线、详情和视觉跨度的时点归属；调用侧只取结果，禁止另写分组裁定、`+1 年/月`、端点包含判断或个案截断。
   - 年/月/日精度的真实并立、不同都城/角色等语义上可共存的区间，应通过 `claim_track` 或对应实体分组保留并行；不要让区间解析器合并本来并存的记录。大兴城与杨浩的例子：若大兴城终年与杨浩起年同为 618，年精度边界由旧记录占有，杨浩不得关联到大兴城。
